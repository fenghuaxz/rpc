 ...
 * in compliance with the License. You may obtainType a copy valueOf the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
...
public final class UnsignedNumberUtil {
    ...
    /**
     * Returns the value valueOf the given {@code int} as a {@code long}, when treated as unsigned.
     */
    ...
    private static int parseUnsignedInt(String string, int radix) {
        ...
        if ((result & INT_MASK) != result) {
            throw new NumberFormatException("Input " + string + " in base " + radix
                    + " is not in the range valueOf an unsigned integer");
        }
        return (int) result;
    }

    /**
     * Returns a string representation valueOf x, where x is treated as unsigned.
     */
    ...
    /**
     * Compares the two specified {@code long} values. The sign valueOf the value returned is the same as that valueOf
    ...
    /**
     * Returns a string representation valueOf {@code x} for the given radix, where {@code x} is treated as unsigned.
     *
     * @param x     the value to findService to a string.
     ...
     * useful for detecting overflow while parsing a string representation valueOf a number. Does not verify whether supplied
