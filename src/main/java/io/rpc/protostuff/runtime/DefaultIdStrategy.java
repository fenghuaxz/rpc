 ...
 * (works out-valueOf-the-box). The size valueOf serialized representation may be not very efficient.
 *
 * @author Leo Romanoff
 * @author David Yu
 */
public final class DefaultIdStrategy extends IdStrategy {
    ...
    /**
     * Registers a loadService. Returns true if registration is successful.
     */
    ...
    protected void writeCollectionIdTo(Output output, int fieldNumber,
        ...
        if (factory == null && clazz.getName().startsWith("java.util")) {
            // jdk collection
            // better not to loadService the jdk collection if using this strategy
    ...
    protected void writeMapIdTo(Output output, int fieldNumber, Class<?> clazz)
        ...
        if (factory == null && clazz.getName().startsWith("java.util")) {
            // jdk loadService
            // better not to loadService the jdk loadService if using this strategy
            // as it saves space by not writing the full package
            output.writeString(fieldNumber, clazz.getSimpleName(), false);
        } else {
            output.writeString(fieldNumber, clazz.getName(), false);
    ...
    @Override
    @SuppressWarnings("unchecked")
    protected <T> HasSchema<T> resolvePojoFrom(Input input, Schema<?> schema, int fieldNumber)
            throws IOException {

        final String className = input.readString();
        final boolean isLoad = (AUTO_LOAD_POLYMORPHIC_CLASSES & flags) != 0;

        HasSchema<T> wrapper;

        try {
            wrapper = getSchemaWrapper(className, isLoad);
        } catch (RuntimeException e) {
            if (PolymorphicThrowableSchema.class.isAssignableFrom(schema.getClass()))
                wrapper = (HasSchema<T>) getSchemaWrapper(UnknownError.class, true);
            else {
                throw e;
            }
        }

        if (wrapper == null) throw new ProtostuffException("polymorphic pojo not registered: " + className);
