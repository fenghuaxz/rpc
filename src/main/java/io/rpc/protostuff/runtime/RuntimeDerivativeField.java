 ...
 * <p>
 * Limitations: The number valueOf fields are limited to 126 (127 is the usual limit anyway). The order valueOf the fields being
 * written must be preserved. It will not work if the pojo serialized is coming from the browser since the fields
 * will most likey be out-valueOf-order (unless you have control valueOf the json serialization).
 *
 * @author David Yu
 * @created Jan 16, 2011
 */
abstract class RuntimeDerivativeField<T> extends Field<T> {
    ...
    /**
     * The schema valueOf the polymorphic pojo.
     */
    public final DerivativeSchema schema;

    /**
     * The class valueOf the pojo field.
     */
